...
func apply_atmosphere() -> void:
	...
	if _environment_node != null and _environment_node.environment != null:
		...
		env.ambient_light_color = clear.lerp(Color(1.0, 0.95, 0.88), daylight * 0.18 + warm_hour * 0.18)
		env.ambient_light_energy = lerpf(0.14, 0.52, daylight) + blue_hour * 0.16 + warm_hour * 0.10
...
func _current_hour() -> float:
	if _population != null and _population.has_method("get_population_summary"):
