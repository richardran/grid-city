...
func _init() -> void:
	...
	var dark:int=0; var bright:int=0; var warm:int=0; var total_windows:int=0
	for p in profiles:
		var cat = str(p.get("category","?"))
		if cat != "house_window" and cat != "_": continue
		total_windows += 1
	...
	print("Window profiles: %d  Dark: %d  Lit: %d  Warm: %d" % [total_windows, dark, bright, warm])
	if warm > total_windows * 0.5: print("OK: Warm windows confirmed")
	else: print("FAIL: Windows not warm enough (only %d/%d)" % [warm, total_windows])
